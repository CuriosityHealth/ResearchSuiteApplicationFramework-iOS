...
public extension URL {
    public var queryDictionary: [String: String]? {
        guard let query = URLComponents(string: self.absoluteString)?.percentEncodedQuery else { return nil}
        
        var queryStrings = [String: String]()
        for pair in query.components(separatedBy: "&") {
            
            guard let key = pair.components(separatedBy: "=")[0].removingPercentEncoding,
                let value = pair
